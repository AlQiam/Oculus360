...
// **** These pixels.h or pixels_1.h contain the pixels of the image
#include "pixels_1.h"
#include <stdio.h>
#include <stdlib.h>
#include <stdbool.h>
...
ovrScene_Create(AAssetManager* amgr, XrInstance instance, XrSession session, ovrScene* scene) {
    ...
    // Simple checkerboard pattern.
    {
        ...
        swapChainCreateInfo.usageFlags =
                XR_SWAPCHAIN_USAGE_SAMPLED_BIT | XR_SWAPCHAIN_USAGE_COLOR_ATTACHMENT_BIT;
        swapChainCreateInfo.format = GL_SRGB8_ALPHA8; //GL_RGBA8; //GL_SRGB8_ALPHA8;
        swapChainCreateInfo.sampleCount = 1;
        swapChainCreateInfo.width = width;
        swapChainCreateInfo.height = height;
        swapChainCreateInfo.faceCount = 1;
        ...
        // **** This nested for-loop is added to read the pixels from pixels_1.h or pixels.h
        for (int y = 0; y < height; y++) {
            for (int x = 0; x < width; x++) {
                texData[y * width + x] = pixels_1[y*width + x];
            }
        }
...
//                if (y == 350 | y == 349 | y == 351 | y == 0 | y == 511 | y == 10 | y == 501){
//                    texData[y * width + x] = 0xFFFF0000;
//                    continue;
//                }
//                texData[y * width + x] = (x ^ y) & 16 ? 0xFFFFFFFF : (green | 0xFF000000 | red);
//            }
//        }
...
//    {
//        static const int QUAD_WIDTH = 1024;
//        static const int QUAD_HEIGHT = 512;
//
//        XrSwapchainCreateInfo swapChainCreateInfo;
//        memset(&swapChainCreateInfo, 0, sizeof(swapChainCreateInfo));
//        swapChainCreateInfo.type = XR_TYPE_SWAPCHAIN_CREATE_INFO;
...
//                texData[y * QUAD_WIDTH + x] = pixels[y*QUAD_WIDTH + x];
//            }
//        }
//
////        for (int y = 0; y < QUAD_HEIGHT; y++) {
...
void android_main(struct android_app* app) {
    ...
    const int my_width = 1024;
    const int my_height = 512;

//    my_texData = (uint32_t*)malloc(my_width * my_height * sizeof(uint32_t));

//    FILE* infile = fopen("/storage/emulated/0/pixels.txt", "r");

//    for (int y = 0; y < my_height; y++) {
//        for (int x = 0; x < my_width; x++) {
//            uint32_t* value = (uint32_t*)malloc(sizeof(uint32_t));
//            uint32_t value;
//            my_texData[y * my_width + x] = pixels[y * my_width + x];
//        }
//    }
//    fclose(infile);
...
//        }
        if (appState.Scene.BackGroundType == BACKGROUND_EQUIRECT) {
            ...
            // **** I changed these three angles which are responsible to make the view 360
            const float centralHorizontalAngle = (2.0f * MATH_PI);
            const float upperVerticalAngle = -(MATH_PI / 2.0f);
            const float lowerVerticalAngle = +MATH_PI/2.0f;
            // **** The upperVerticalAngle is MINUS 90 and the lowerVerticalAngle is PLUS 90
            // **** This necessary because OpenGL starts reading from bottom left of the angle
            // **** Thus, the image should be flipped vertically, and this is sufficient to do so.
...
//                XrQuaternionf_CreateFromVectorAngle(axis, 90.0f * MATH_PI / 180.0f);
